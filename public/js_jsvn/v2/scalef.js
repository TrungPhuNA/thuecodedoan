...
let Scalef = (function () {
    ...
    const init = function (customOptions) {
            ...
            if (adSpaceIdValue === '') {
                ...
                if (clickIdValue === '') {
                    if (!getCookie('csf_click_id')) {
                        ...
                        clickIdValue = getCookie('csf_click_id');
                    }
                }
                setCookie('csf_click_id', clickIdValue, 30);
            }
        } else {
        ...
        if (!options.network_id) {
            ...
            if (networkIdValue === '') {
                options.error({
                    message: 'The network_id is required.'
                });
                return;
            }
        } else {
            networkIdValue = options.network_id;
        }
    ...
    const validateFormData = function (success, error = options.error) {
        ...
        if (phoneValue === getCookie('csf_phone')) {
            const now = new Date().getTime();
            const expires = new Date(getCookie('csf_phone_expires')).getTime();
    ...
    const createConversion = function () {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (response.status === 'error') {
                    ...
                    setCookie('csf_phone', phoneValue, options.cookie_expires);

                    const now = new Date().getTime();
                    const expires = new Date(now + (options.cookie_expires * 86400000));
                    setCookie('csf_phone_expires', expires.toUTCString());
        ...
        xhttp.send('name=' + nameValue + '&phone=' + phoneValue + '&note=' + noteValue + '&network_id=' + networkIdValue + '&price_index=' + options.price_index);
    };
