...
let D2C = (function () {
    ...
    let errorMessages = {
        en: {
            ...
            email_phone_required: {
                required: 'The phone or email field is required.',
                regex: 'The phone or email format is invalid.',
                exist: 'The selected phone or email already exists in the system. Please wait 3 days to continue. '
            },
            note: {
        ...
        vi: {
            ...
            email_phone_required: {
                required: 'Trường email hoặc điện thoại là bắt buộc.',
                regex: 'Định dạng email hoặc điện thoại không hợp lệ.',
                exist: 'Email hoặc điện thoại được chọn đã tồn tại trong hệ thống. Vui lòng đợi 3 ngày để tiếp tục. '
            },
            note: {
    ...
    const setFormData = function (submit) {
        ...
        // phone
        if (form.querySelector(options.element_phone)) {
        ...
        // email
        if (form.querySelector(options.element_email)) {
    ...
    const validateFormData = function (success, error = options.error) {
        ...
        if (phoneValue === '' && emailValue === '') {
            console.log(errorMessages[options.language].email_phone_required.required);
            errors.phone.push(errorMessages[options.language].email_phone_required.required);
        }
        /*if (phoneValue !== '' && !/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{4})$/.test(phoneValue)) {
            console.log(errorMessages[options.language].phone.regex);
            errors.phone.push(errorMessages[options.language].phone.regex);
        }*/
        if (emailValue !== '' && checkEmail(emailValue) === false) {
            console.log(errorMessages[options.language].email.regex);
            errors.email.push(errorMessages[options.language].email.regex);
        }
        if (phoneValue !== '' && phoneValue === getCookie('d2c_phone')) {
        ...
        if (emailValue !== '' && emailValue === getCookie('d2c_email')) {
            const now = new Date().getTime();
        ...
        if (errors.name.length > 0 || errors.phone.length > 0 || errors.email.length > 0) {
            error(errors);
        } else {
            success();
        }
    ...
    const checkEmail = function(email) {
        emailRegExp = /^[A-Z_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.([a-z]){2,4})$/;
    ...
    const createConversion = function () {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (response.status === 'error') {
                    ...
                    // xóa cookie và utm_source khi reoccur == 0
                    if (response.data.reoccur === 0 || response.data.reoccur === '0') {
                        setCookie('d2c_click_id', '', 0);
        ...
        const paramEmail =  (emailValue !== '') ? '&email=' + emailValue : '';
        const paramPhone = (phoneValue !== '') ? '&phone=' + phoneValue : '';
        xhttp.send('name=' + nameValue + paramPhone + paramEmail + '&note=' + noteValue + '&quantity=' + quantityValue + '&price_index=' + options.price_index);
    };
